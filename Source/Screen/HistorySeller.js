...
const HistorySeller = () => {
    ...
    return (
        <View className='h-full w-full mb-6 px-4' style={{ backgroundColor: Colors.dark.colors.subbackGroundColor }}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                {historyOutlet.length === 0 ? (
                    <View className="flex-1 justify-center items-center p-2" style={{ height: Dimensions.get('window').height * 1 }}>
                        <Text className="font-black text-xl text-center py-3" style={{ color: Colors.dark.colors.mainTextColor }}>No order history available</Text>
                    </View>
                ) : (
        ...
        <View style={styles.orderContainer}>
            ...
            <TouchableOpacity onPress={() => setShowDetails(!showDetails)} className='flex-row justify-end rounded-md items-end' style={{  borderWidth: 0, borderColor: Colors.dark.colors.diffrentColorOrange }}>
                ...
                <View className=' items-end justify-end ml-1 mb-2'>
                    ...
                    <Ionicons className=' bottom-0 right-0' name={showDetails ? 'caret-up' : 'caret-down'} size={14} color={Colors.dark.colors.textColor} />
                </View>
            </TouchableOpacity>
