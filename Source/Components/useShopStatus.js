...
const useShopStatus = (openingTime = '09:00 am', closingTime = '09:00 pm', offDays = [], leaveDayString = '') => {
    ...
    if (isLeaveDay) {
      ...
      if (currentTime >= openingMinutes || currentTime < closingMinutes) {
        if (currentTime >= closingMinutes - 60 && closingMinutes - currentTime > -60) {
          newStatus = {
            text: `Hurry up! Kitchen's closing in ${closingMinutes - currentTime} minutes! Get your order now, before it's too late!`, color: ["#f7ff00", '#1A4314'], state: "closingSoon"
          }
        } else {
          newStatus = {
            text: "We're open and ready to serve! Dive into our menu now and enjoy a delicious meal!", color: ['#00FF00', '#1A4314'], state: "open"
          }
        }
      } else {
        if (currentTime >= openingMinutes - 60 && openingMinutes - currentTime > -60) {
          newStatus = {
            text: `Almost there! Just ${openingMinutes - currentTime} minutes until the kitchen's back in action! Get ready to place your order!`, color: ["#f7ff00", '#FF0000'], state: "openingSoon"
          }
        } else {
          newStatus = {
            text: `Oops! You missed us! We're closed for now, but we'll be back at ${openingTime}.`, color: ["#FF0000", Colors.dark.colors.backGroundColor],
            state: "closed"
          };
        }
      }
      ...
      if (currentTime >= openingMinutes && currentTime < closingMinutes) {
        if (currentTime >= closingMinutes - 60) {
          newStatus = {
            text: `Hurry up! Kitchen's closing in ${closingMinutes - currentTime} minutes! Get your order in now before it's too late!`, color: ["#f7ff00", '#1A4314'], state: "closingSoon"
          }
        } else {
          newStatus = {
            text: "We're open and ready to serve! Dive into our menu now and enjoy a delicious meal!", color: ['#00FF00', '#1A4314'], state: "open"
        ...
        if (currentTime >= openingMinutes - 60 && openingMinutes - currentTime > -60) {
          newStatus = {
            text: `Almost there! Just ${openingMinutes - currentTime} minutes until the kitchen's back in action!`, color: ["#f7ff00", '#FF0000'], state: "openingSoon"
          }
        } else {
          newStatus = {
            text: `Oops! You missed us! We're closed for now, but we'll be back at ${openingTime}.`, color: ["#FF0000", Colors.dark.colors.backGroundColor],
